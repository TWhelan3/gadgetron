...
public:
	BipolarCorrectMatlabGadget(): Gadget1<IsmrmrdImageArray>()
	{
		...
		if (!(engine_ = engOpen("matlab -nosplash -nodesktop"))) {
			...
			std::string gadgetron_matlab_path = get_gadgetron_home() + "/gadgets/matlab";//"/share/gadgetron/matlab";  Authours made a weird installation assumption here I think
