#include "MultiChanReconGadget.h"
#include "hoNDFFT.h"
#include "hoNDArray_math.h"

namespace Gadgetron{

int MultiChanReconGadget::process( GadgetContainerMessage<IsmrmrdReconData>* m1)
{
        ...
        it != m1->getObjectPtr()->rbit_.end(); ++it)
    {
        ...
        for (uint16_t loc=0; loc < LOC; loc++) {
		    ...
		    hoNDFFT<float>::instance()->fft3c(chunk);
		    ...
		    output.copyFrom(chunk);
                    //Zero out the output
                    //clear(output);
		    //imarray.data_(0,0,0,0,n,s,loc)= chunk(0,0,0,0);
                    //Compute d* d in place
                    //multiplyConj(chunk,chunk,chunk);                    
                    //Add up
                   //for (size_t c = 0; c < CHA; c++) {
                     //   output(0,0,0,c) = chunk(0,0,0,c);
                   //}                    
...
GADGET_FACTORY_DECLARE(MultiChanReconGadget)
}
